...
test('expanded popular markets resolve without aliasing old tokens or changing source', async () => {
  ...
  assert.equal(constants.FOREX_SYMBOLS.length, 43); assert.equal(constants.CRYPTO_SYMBOLS.length, 31);
...
test('new crypto CoinGecko identities are exact; old MATIC identity stays separate', () => {
  ...
  for (const [symbol, id] of Object.entries({ BCH: 'bitcoin-cash', TRX: 'tron', SUI: 'sui', NEAR: 'near', UNI: 'uniswap', AAVE: 'aave', POL: 'polygon-ecosystem-token', ATOM: 'cosmos', ETC: 'ethereum-classic', XLM: 'stellar', FIL: 'filecoin', ARB: 'arbitrum', OP: 'optimism', INJ: 'injective-protocol', ICP: 'internet-computer', APT: 'aptos', HBAR: 'hedera-hashgraph', VET: 'vechain', ALGO: 'algorand' })) {
